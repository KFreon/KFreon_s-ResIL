	...
	ILubyte*& CompData)
{
	...
	FILE* log = fopen("ResIL_LOG_DDSdecompress.txt", "a");
	fprintf(log, "Beginning dds decompress\n");
	fflush(log);
...
INT Adjustment(int format)
{
	int adjustment = PF_DXT1;
	switch (format)
	{
	case 0:
		adjustment = -5;
		break;
	case 7:
		adjustment = -PF_DXT1 + 1;  // KFreon: 7 - (-1) = 8 ---> 8 + IL_DXT1 == IL_3Dc
		break;
	case 8:
		adjustment = -2;
		break;
	...
	ILboolean Has16BitComponents, SIO * io)
{
	...
	FILE* log = fopen("ResIL_LOG_Readmips.txt", "a");
	fprintf(log, "starting readmimaps\n");
	fflush(log);
	...
	StartImage = image;
	image->DxtcFormat = CompFormat - Adjustment(CompFormat) + IL_DXT1;
	...
	for (i = 0; i < header->MipMapCount - 1; i++) {
		...
		if (il2GetInteger(IL_KEEP_DXTC_DATA) == IL_TRUE && isCompressed == IL_TRUE && CompData) {
			...
			fprintf(log, "dxtc data %p\n", image->DxtcData);
			fflush(log);
			if (image->DxtcData == NULL)
				return IL_FALSE;

			// KFreon: Set formats and adjustments - These convert from the PF_DXT1 enum to the IL_DXT1 definitions
			
			image->DxtcFormat = CompFormat - Adjustment(CompFormat) + IL_DXT1;

			fprintf(log, "compformat %d    format %d\n", CompFormat, image->DxtcFormat);
			fflush(log);
			image->DxtcSize = header->LinearSize;
			memcpy(image->DxtcData, CompData, image->DxtcSize);
		}
...
ILboolean iLoadDdsInternal(ILimage* baseImage)
{
	...
	FILE* log = fopen("ResIL_LOG_DDSload.txt", "a");
	fprintf(log, "format %d\n\n\n\n", CompFormat);
	fclose(log);
	...
	FILE* log2 = fopen("ResIL_LOG_DDSload.txt", "a");
	fprintf(log2, "baseimage format: %d   currImage format: %d\n\n\n\n", baseImage->DxtcFormat, currImage->DxtcFormat);
	fclose(log2);
	return il2FixImage(baseImage);
}
