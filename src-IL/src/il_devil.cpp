...
ILimage* ILAPIENTRY il2GetMipmap(ILimage* image, ILuint Number)
{
	...
	/*fprintf(log, "number isn't 0\n");
	fflush(log);*/

	// KFreon: Since mipcount doesn't work properly, possibility of Mipmaps being null. 
	if (image->Mipmaps == NULL)
	{
		return NULL;
	}
	...
	/*fprintf(log, "mips set to io or something...\n");
	fflush(log);*/

	image = image->Mipmaps;

	/*fprintf(log, "mip set to image...\n");
	fflush(log);*/
	...
	/*fprintf(log, "mip isnt null?\n");
	fflush(log);*/

	for (ILuint Current = 1; Current < Number; Current++) {
		/*fprintf(log, "Current = %d\n", Current);
		fflush(log);

		fprintf(log, "image->mipmaps:  %p\n", image->Mipmaps);
		fflush(log);

		fprintf(log, "Image->mipmaps->io:  %p\n", image->Mipmaps->io);
		fflush(log);

		fprintf(log, "image->io:  %p\n", image->io);
		fflush(log);*/
		...
		/*fprintf(log, "Reassigned some mips\n", Current);
		fflush(log);*/

		image = image->Mipmaps;

		/*fprintf(log, "Reassigned image\n", Current);
		fflush(log);*/
		if (image == NULL) {
			/*fprintf(log, "image is null");
			fflush(log);*/
			il2SetError(IL_ILLEGAL_OPERATION);
			return NULL;
		}
		/*fprintf(log, "Current image isn't null for Current = %d\n", Current);
		fflush(log);*/
	}
	/*fprintf(log, "Done\n\n\n");
	fclose(log);*/
