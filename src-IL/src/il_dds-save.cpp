...
ILuint GetCubemapInfo(ILimage* image, ILint* faces)
{
	...
	il2GetImageInteger(image, IL_NUM_IMAGES, (ILint*) &srcImagesCount);
	if (srcImagesCount != 5) //write only complete cubemaps (TODO?)
		return 0;

	img = image;
	il2GetImageInteger(image, IL_NUM_MIPMAPS, (ILint*) &srcMipmapCount);
	...
	for (i = 0; i < 6; ++i) {
		...
		il2GetImageInteger(img, IL_NUM_MIPMAPS, (ILint*) &srcMipmapCount);
...
ILubyte *CompressTo88(ILimage *curImage)
{
	ILimage		*TempImage;
	ILubyte		*Data;
	ILuint		i, j;

	if ((curImage->Type != IL_UNSIGNED_BYTE && curImage->Type != IL_BYTE) || curImage->Format == IL_COLOUR_INDEX) {
		TempImage = iConvertImage(curImage, IL_BGR, IL_UNSIGNED_BYTE);  // @TODO: Needs to be BGRA.
		if (TempImage == NULL)
			return NULL;
	}
	else {
	...
	Data = (ILubyte*)ialloc(curImage->Width * curImage->Height * 2 * curImage->Depth);
	if (Data == NULL) {
		if (TempImage != curImage)
...
void CompressToRXGB(ILimage *curImage, ILushort** xgb, ILubyte** r)
{
	ILimage		*TempImage;
	ILuint		i, j;
	ILushort	*Data;
	...
	*xgb = (ILushort*)ialloc(curImage->Width * curImage->Height * 2 * curImage->Depth);
	*r = (ILubyte*)ialloc(curImage->Width * curImage->Height * curImage->Depth);
	if (*xgb == NULL || *r == NULL) {
		if (TempImage != curImage)
			ilCloseImage(TempImage);
		return;
...
ILushort *CompressTo565(ILimage *curImage)
{
	ILimage		*TempImage;
	ILushort	*Data;
	ILuint		i, j;
	...
	if ((curImage->Type != IL_UNSIGNED_BYTE && curImage->Type != IL_BYTE) || curImage->Format == IL_COLOUR_INDEX) {
		TempImage = iConvertImage(curImage, IL_BGRA, IL_UNSIGNED_BYTE);  // @TODO: Needs to be BGRA.
		if (TempImage == NULL)
			return NULL;
	}
	else {
	...
	Data = (ILushort*)ialloc(curImage->Width * curImage->Height * 2 * curImage->Depth);
	if (Data == NULL) {
		if (TempImage != curImage)
			ilCloseImage(TempImage);
		return NULL;
	...
	switch (TempImage->Format)
	{
		...
		case IL_ALPHA:
			memset(Data, 0, curImage->Width * curImage->Height * 2 * curImage->Depth);
			break;
	}
...
ILuint Compress(ILimage *curImage, ILenum DXTCFormat)
{
	ILushort	*Data, Block[16], ex0, ex1, *Runner16, t0, t1;
	ILuint		x, y, z, i, BitMask;
	ILubyte		*Alpha, AlphaBlock[16], AlphaBitMask[6], a0, a1;
	ILboolean	HasAlpha;
	ILuint		Count = 0;
	ILubyte		*Data3Dc, *Runner8;

	if (DXTCFormat == IL_3DC) {
		Data3Dc = CompressTo88(curImage);
		if (Data3Dc == NULL)
			return 0;

		Runner8 = Data3Dc;

		for (z = 0; z < curImage->Depth; z++) {
			for (y = 0; y < curImage->Height; y += 4) {
				for (x = 0; x < curImage->Width; x += 4) {
					Get3DcBlock(AlphaBlock, Runner8, curImage, x, y, 0);
					ChooseAlphaEndpoints(AlphaBlock, &a0, &a1);
					GenAlphaBitMask(a0, a1, AlphaBlock, AlphaBitMask, NULL);
					curImage->io.putc(a0, &curImage->io);
					curImage->io.putc(a1, &curImage->io);
					curImage->io.write(AlphaBitMask, 1, 6, &curImage->io);

					Get3DcBlock(AlphaBlock, Runner8, curImage, x, y, 1);
					ChooseAlphaEndpoints(AlphaBlock, &a0, &a1);
					GenAlphaBitMask(a0, a1, AlphaBlock, AlphaBitMask, NULL);
					curImage->io.putc(a0, &curImage->io);
					curImage->io.putc(a1, &curImage->io);
			...
			Runner8 += curImage->Width * curImage->Height * 2;
		}
		ifree(Data3Dc);
	}
	...
	else if (DXTCFormat == IL_ATI1N)
	{
		...
		if (curImage->Bpp != 1) {
			TempImage = iConvertImage(curImage, IL_LUMINANCE, IL_UNSIGNED_BYTE);
			if (TempImage == NULL)
				return 0;
		}
		else {
		...
		for (z = 0; z < curImage->Depth; z++) {
			for (y = 0; y < curImage->Height; y += 4) {
				for (x = 0; x < curImage->Width; x += 4) {
					GetAlphaBlock(AlphaBlock, Runner8, curImage, x, y);
					ChooseAlphaEndpoints(AlphaBlock, &a0, &a1);
					GenAlphaBitMask(a0, a1, AlphaBlock, AlphaBitMask, NULL);
					curImage->io.putc(a0, &curImage->io);
					curImage->io.putc(a1, &curImage->io);
			...
			Runner8 += curImage->Width * curImage->Height;
		}
	...
	else
	{
		...
		if (DXTCFormat != IL_RXGB) {
			Data = CompressTo565(curImage);
			if (Data == NULL)
				return 0;

			Alpha = il2GetAlpha(curImage, IL_UNSIGNED_BYTE);
			if (Alpha == NULL) {
				ifree(Data);
				return 0;
			}
			...
			CompressToRXGB(curImage, &Data, &Alpha);
				...
				for (z = 0; z < curImage->Depth; z++) {
					for (y = 0; y < curImage->Height; y += 4) {
						for (x = 0; x < curImage->Width; x += 4) {
							...
							GetBlock(Block, Runner16, curImage, x, y);
							ChooseEndpoints(Block, &ex0, &ex1);
							CorrectEndDXT1(&ex0, &ex1, HasAlpha);
							SaveLittleUShort(&curImage->io, ex0);
							SaveLittleUShort(&curImage->io, ex1);
							if (HasAlpha)
								BitMask = GenBitMask(ex0, ex1, 3, Block, AlphaBlock, NULL);
							else
								BitMask = GenBitMask(ex0, ex1, 4, Block, NULL, NULL);
							SaveLittleUInt(&curImage->io,BitMask);
							Count += 8;
						}
					}

					Runner16 += curImage->Width * curImage->Height;
					Runner8 += curImage->Width * curImage->Height;
				}
				break;
			...
			/*case IL_DXT2:
				for (y = 0; y < Image->Height; y += 4) {
					for (x = 0; x < Image->Width; x += 4) {
						...
						for (i = 0; i < 16; i += 2) {
							curImage->io.putc(curImage->io.handle, (ILubyte)(((AlphaBlock[i] >> 4) << 4) | (AlphaBlock[i+1] >> 4)));
						}
				...
				for (z = 0; z < curImage->Depth; z++) {
					for (y = 0; y < curImage->Height; y += 4) {
						for (x = 0; x < curImage->Width; x += 4) {
							GetAlphaBlock(AlphaBlock, Runner8, curImage, x, y);
							for (i = 0; i < 16; i += 2) {
								curImage->io.putc((ILubyte)(((AlphaBlock[i+1] >> 4) << 4) | (AlphaBlock[i] >> 4)), &curImage->io);
							}
...
ILboolean iSaveDdsInternal(ILimage* curImage)
{
	...
	FILE* log = fopen("ResIL_LOG_DDSsave.txt", "a");
	fprintf(log, "beginning...\n");
	fflush(log);

	CubeFlags = GetCubemapInfo(curImage, CubeTable);

	//ILuint image = ilGetCurName();
	DXTCFormat = iGetInt(IL_DXTC_FORMAT);
	fprintf(log, "DXT Format: %d\n", DXTCFormat);
	fflush(log);

	fprintf(log, "------Before writing header: %d\n", curImage->io.tell(&curImage->io));
	WriteHeader(curImage, DXTCFormat, CubeFlags);
	fprintf(log, "------After writing header: %d\n", curImage->io.tell(&curImage->io));

	if (CubeFlags != 0)
		numFaces = il2GetInteger(IL_NUM_FACES); // Should always be 5 for now
	else
		numFaces = 0;

	numMipMaps = il2GetInteger(IL_NUM_MIPMAPS); //this assumes all faces have same # of mipmaps
	numMipMaps = 13;


	fprintf(log, "numFaces: %d  nummips: %d\n", numFaces, numMipMaps);
	...
	for (i = 0; i <= numFaces; ++i) {
		for (counter = 0; counter <= numMipMaps; counter++) {
			...
			fprintf(log, "================Mip level: %d\n", counter);
			//fprintf(log, "curImage pointer before getframe: %p\n", curImage);
			fflush(log);

			//fprintf(log, "------Before getting frame: %d\n", curImage->io.tell(&curImage->io));
			ILimage* frame = il2GetFrame(curImage, CubeTable[i]);
			//fprintf(log, "------After getting frame: %d\n", curImage->io.tell(&curImage->io));
			//fprintf(log, "After frame\n");
			//fflush(log);
			//fprintf(log, "Frame pointer: %p\n", frame);
			fprintf(log, "currImage pointer: %p\n", curImage);
			//fflush(log);

			//fprintf(log, "------FRAME - Before getting mipmap %d\n", frame->io.tell(&frame->io));
			ILimage* mipmap = il2GetMipmap(frame, counter);
			//fprintf(log, "------FRAME - After getting mipmap: %d\n", frame->io.tell(&frame->io));
			fprintf(log, "BEFORE SETTING IO === frame IO: %p   Mipmap IO %p\n", &frame->io, &mipmap->io);

			fprintf(log, "mipmap pointer: %p\n", mipmap);
			fflush(log);

			if (mipmap == NULL)
				break;

			fprintf(log, "AFTER SETTING IO === frame IO: %p   Mipmap IO %p\n", &frame->io, &mipmap->io);
			
			//fprintf(log, "------MIPMAP - Before first origin check: %d\n", mipmap->io.tell(&mipmap->io));
			if (mipmap->Origin != IL_ORIGIN_UPPER_LEFT) {
				mipmap->Data = iGetFlipped(curImage);
				if (mipmap->Data == NULL) {
					mipmap->Data = CurData;
					return IL_FALSE;
				}
			}
			fprintf(log, "------MIPMAP - After first origin check: %d\n", mipmap->io.tell(&mipmap->io));

			//fprintf(log, "after origin\n");
			//fflush(log);

			fprintf(log, "------MIPMAP - Before compress: %d\n", mipmap->io.tell(&mipmap->io));
			if (!Compress(mipmap, DXTCFormat))
				return IL_FALSE;
			fprintf(log, "------MIPMAP - After compress: %d\n", mipmap->io.tell(&mipmap->io));

			//fprintf(log, "after compress\n");
			//fflush(log);

			//fprintf(log, "------MIPMAP - Before second origin check: %d\n", mipmap->io.tell(&mipmap->io));
			if (mipmap->Origin != IL_ORIGIN_UPPER_LEFT) {
				ifree(mipmap->Data);
				mipmap->Data = CurData;
			}
			fprintf(log, "------MIPMAP - After Second origin check: %d\n\n", mipmap->io.tell(&mipmap->io));
			//fprintf(log, "after origin left\n");
...
ILboolean WriteHeader(ILimage *curImage, ILenum DXTCFormat, ILuint CubeFlags)
{
	...
	switch (DXTCFormat)
	{
		...
		default:
			// Error!
			il2SetError(IL_INTERNAL_ERROR);  // Should never happen, though.
			return IL_FALSE;
	}

	curImage->io.write("DDS ", 1, 4, &curImage->io);
	SaveLittleUInt(&curImage->io,124);		// Size1
	SaveLittleUInt(&curImage->io,Flags1);		// Flags1
	SaveLittleUInt(&curImage->io,curImage->Height);
	SaveLittleUInt(&curImage->io,curImage->Width);
	...
	LinearSize = (((curImage->Width + 3)/4) * ((curImage->Height + 3)/4)) * BlockSize * curImage->Depth;
	...
	*/
	...
	SaveLittleUInt(&curImage->io,LinearSize);	// LinearSize (TODO: change this when uncompressed formats are supported)

	if (curImage->Depth > 1) {
		SaveLittleUInt(&curImage->io, curImage->Depth);			// Depth
		ddsCaps2 |= DDS_VOLUME;
	}
	else
		SaveLittleUInt(&curImage->io,0);						// Depth


	// KFreon: Count Mips
	ILimage* mipper = curImage;
	int mipcount = 1;
	while (mipper->Mipmaps != NULL)
	{
		mipper = mipper->Mipmaps;
		mipcount++;
	}



	SaveLittleUInt(&curImage->io, mipcount);  // MipMapCount
	SaveLittleUInt(&curImage->io,0);			// AlphaBitDepth

	for (i = 0; i < 10; i++)
		SaveLittleUInt(&curImage->io,0);		// Not used

	SaveLittleUInt(&curImage->io,32);			// Size2
	SaveLittleUInt(&curImage->io,Flags2);		// Flags2
	SaveLittleUInt(&curImage->io,FourCC);		// FourCC
	SaveLittleUInt(&curImage->io,0);			// RGBBitCount
	SaveLittleUInt(&curImage->io,0);			// RBitMask
	SaveLittleUInt(&curImage->io,0);			// GBitMask
	SaveLittleUInt(&curImage->io,0);			// BBitMask
	SaveLittleUInt(&curImage->io,0);			// RGBAlphaBitMask
	ddsCaps1 |= DDS_TEXTURE;
	//changed 20040516: set mipmap flag on mipmap images
	//(non-compressed .dds files still not supported, though)
	...
	SaveLittleUInt(&curImage->io,ddsCaps1);	// ddsCaps1

	SaveLittleUInt(&curImage->io,ddsCaps2);	// ddsCaps2
	SaveLittleUInt(&curImage->io,0);			// ddsCaps3
	SaveLittleUInt(&curImage->io,0);			// ddsCaps4
	SaveLittleUInt(&curImage->io,0);			// TextureStage
...
ILuint ILAPIENTRY il2GetDXTCData(ILimage* curImage, void *Buffer, ILuint BufferSize, ILenum DXTCFormat)
{
	ILubyte	*CurData = NULL;
	ILuint	retVal;
	ILint	BlockNum;
	...
	if (Buffer == NULL) {  // Return the number that will be written with a subsequent call.
		BlockNum = ((curImage->Width + 3)/4) * ((curImage->Height + 3)/4)
					* curImage->Depth;
	...
	if (DXTCFormat == curImage->DxtcFormat && curImage->DxtcSize && curImage->DxtcData) {
		memcpy(Buffer, curImage->DxtcData, IL_MIN(BufferSize, curImage->DxtcSize));
		return IL_MIN(BufferSize, curImage->DxtcSize);
	}

	if (curImage->Origin != IL_ORIGIN_UPPER_LEFT) {
		CurData = curImage->Data;
		curImage->Data = iGetFlipped(curImage);
		if (curImage->Data == NULL) {
			curImage->Data = CurData;
			return 0;
		}
	}

	//@TODO: Is this the best way to do this?
	iSetOutputLump(&curImage->io, Buffer, BufferSize);
	retVal = Compress(curImage, DXTCFormat);
...
ILAPI ILubyte* ILAPIENTRY ilCompressDXT(ILimage* curImage, ILubyte *Data, ILuint Width, ILuint Height, ILuint Depth, ILenum DXTCFormat, ILuint *DXTCSize)
{
	ILimage *TempImage, *CurImage = curImage;
	ILuint	BuffSize;
	ILubyte	*Buffer;
	...
	BuffSize = il2GetDXTCData(curImage, NULL, 0, DXTCFormat);
	if (BuffSize == 0)
		return NULL;
