...
ILAPI ILboolean ILAPIENTRY il2LoadFuncs(ILimage* imageExt, ILenum type)
{
	...
	FILE* log = fopen("ResIL_LOG_Loadfuncs.txt", "a");
	...
	FILE* log2 = fopen("ResIL_LOG_Loadfuncs.txt", "a");
	fprintf(log2, "format %d\n\n\n\n",image->DxtcFormat);
	fclose(log2);
...
ILboolean ILAPIENTRY il2Load(ILimage* imageExt, ILenum Type, ILconst_string FileName)
{
	...
	if (image->io.handle != NULL) {
		...
		/*FILE* log = fopen("ResIL_LOG_Loading.txt", "w");
		fprintf(log, "format: %d\n", image->DxtcFormat);
		fflush(log);
		fprintf(log, "dataformat pointer: %p\n", &(image->Format));
		fprintf(log, "mips pointer: %p\n", &(image->Mipmaps));
		fprintf(log, "surface pointer: %p\n", &(image->DxtcFormat));
		fclose(log);*/
...
ILAPI ILboolean ILAPIENTRY il2LoadImage(ILimage* imageExt, ILconst_string FileName)
{
	...
	FILE* log = fopen("ResIL_LOG_Load.txt", "a");

	if (type != IL_TYPE_UNKNOWN)
	{
		bool res = il2Load(image, type, FileName);
		fprintf(log, "Details\n");
		fprintf(log, "addresses: palette %p   duration %p    data %p   datasize %p  bps %p\n", &(image->Pal), &(image->Duration), &(image->Data), &(image->SizeOfData), &(image->Bps));
		fprintf(log, "address of next mipmap %p\n", image->Mipmaps);
		fprintf(log, "size of palette: %p\n", sizeof(image->Pal));
		fprintf(log, "sizeof data: %d  sizeof mips: %d  sizeof Next: %d\n", sizeof(image->Data), sizeof(image->Mipmaps), sizeof(image->Next));
		fprintf(log, "Width: %d  Height: %d  Depth: %d\n", image->Width, image->Height, image->Depth);
		fprintf(log, "bpp: %d  bcc: %d  bpsl: %d\n", image->Bpp, image->Bpc, image->Bps);
		fprintf(log, "datasize: %d  plane: %d  format: %d  type: %d\n", image->SizeOfData, image->SizeOfPlane, image->Format, image->Type);
		fprintf(log, "offsets (x,y): %d %d  duration: %d  cubeflags: %d\n", image->OffX, image->OffY, image->Duration, image->CubeFlags);
		fprintf(log, "mips addr: %p  nextaddr: %p mips: %p  next: %p\n", &(image->Mipmaps), &(image->Next), image->Mipmaps, image->Next);
		fprintf(log, "dxt: %d  size: %d\n", image->DxtcFormat, image->DxtcSize);
		fprintf(log, "surfaceformat address: %p", &(image->DxtcFormat));
		fprintf(log, "\n\n\n\n");
		fclose(log);
		return res;
	}
...
ILAPI ILboolean ILAPIENTRY il2Save(ILimage* imageExt, ILenum type, ILconst_string FileName)
{
	...
	if (image->io.handle != NULL) {
		...
		/*FILE* log = fopen("save.txt", "a");
		fprintf(log, "Before savefuncs: %d\n", image->io.tell(&image->io));*/
...
ILAPI ILboolean ILAPIENTRY il2SaveImage(ILimage* imageExt, ILconst_string FileName)
{
	...
	if (image->io.handle != NULL) {
		...
		/*FILE* file = fopen("R:\\saveimage.txt", "a");
		fprintf(file, "Before savefuncs: %d\n", image->io.tell(&image->io));*/
		bRet = il2SaveFuncs(image, type);
		/*fprintf(file, "After savefuncs: %d\n", image->io.tell(&image->io));
		fclose(file);*/
...
ILAPI ILint64 ILAPIENTRY il2SaveL(ILimage* imageExt, ILenum Type, void *Lump, ILuint Size)
{
	...
	/*FILE* log = fopen("R:\\savelump.txt", "a");
	fprintf(log, "Before savefuncs: %d\n", pos1);*/
	ILboolean bRet = il2SaveFuncs(image, Type);
	/*fprintf(log, "After savefuncs: %d\n", image->io.tell(&image->io));
	fclose(log);*/
